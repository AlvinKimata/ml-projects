...
class MMRLModel(tf.keras.Model):
  ...
  def __init__(self, base_module, head_module, params):
    """MMRLModel.
    Args:
    ...
    """
    super(MMRLModel, self).__init__(name=params['model_config']['model_name'])
    self._params = params

    self._base_layer = base_module(**params['model_config']['backbone_config'])
  ...
  def _build_losses(self, params):
    ...
    for bridge_params in params['model_config']['loss_config']['bridge']:
...
def build_model(params = None, override_params = None,model_arch = None):
  """Build model by name."""
  if params is None:
  ...
  head_module = head_factory.build_model(params=params['model_config']['head_config'])
  model = MMRLModel(
